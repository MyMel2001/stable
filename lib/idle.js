...
${textToSummarize}`;
        ...
        const res = await aiProvider.getDecision(summaryPrompt, "You are a memory manager. Create concise, factual, and strictly anonymized summaries.", abortController.signal);
      ...
      const topicPrompt = `You are an AI that wants to improve itself. What is a topic you should learn about to be a more helpful and knowledgeable assistant?
Consider current events, scientific breakthroughs, or general knowledge that might be useful. Make each query of an unique wealth of topics (Vocaloid, gardening, tech/science breakthroughs, latest news, Goat Simulator tricks - choose any topic at random). Make sure the topics researched are useful for at least one scenario. Make sure the topic is randomly selected.

Respond ONLY with a JSON object:
{"topic": "the topic name", "query": "optimized search query"}`;

      const topicRes = await aiProvider.getDecision(topicPrompt, "You are a self-improvement module. Respond ONLY in JSON.", abortController.signal);
      ...
      if (jsonMatch) {
        ...
        // Scrape top 4 results from sparksammy
        const scrapePromises = sparksammy.slice(0, 4).map(result => scrapeUrl(result.link));
...
${researchContent.substring(0, 20000)}`;
          ...
          const synthesisRes = await aiProvider.getDecision(synthesisPrompt, "You are a research synthesizer. Create a detailed knowledge entry.", abortController.signal);
