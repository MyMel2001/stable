...
${textToSummarize}`;
        ...
        const res = await getDecision(summaryPrompt, "You are a memory manager. Create concise, factual, and strictly anonymized summaries.", abortController.signal);
...
{"topic": "the topic name", "query": "optimized search query"}`;
      ...
      const topicRes = await getDecision(topicPrompt, "You are a self-improvement module. Respond ONLY in JSON.", abortController.signal);
