...
async function searchDuckDuckGo(query) {
  try {
    // DDG Instant Answer API (limited but useful)
...
async function fetchWikipedia(query) {
  try {
    ...
    try {
      const searchRes = await axios.get(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(query)}&limit=1&namespace=0&format=json`);
      if (searchRes.data[1] && searchRes.data[1][0]) {
