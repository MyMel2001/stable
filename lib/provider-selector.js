...
function getProvider() {
  ...
  const providerType = (process.env.AI_PROVIDER || 'ollama').replace(/['"]/g, '').trim().toLowerCase();
  console.log(`[Provider] Value: ${providerType}`)
  if (providerType === 'openai') {
    const decisionModel = process.env.OPENAI_DECISION_MODEL || 'gpt-3.5-turbo';
    const choiceModel = process.env.OPENAI_CHOICE_MODEL || 'gpt-4';
    const numParallel = parseInt(process.env.OPENAI_NUM_PARALLEL, 10) || 0;
    console.log(`[Provider] Using OpenAI provider (${decisionModel}, ${choiceModel}, numParallel=${numParallel || 'auto'})`);
    ...
    provider.decisionModel = decisionModel;
    provider.choiceModel = choiceModel;
    provider.numParallel = numParallel;
    cachedProvider = provider;
  } else {
    console.log(`[Provider] Using Ollama provider (${DECISION_MODEL}, ${CHOICE_MODEL})`);
    cachedProvider = {
      decisionModel: DECISION_MODEL,
      choiceModel: CHOICE_MODEL,
