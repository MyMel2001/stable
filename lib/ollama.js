...
async function getChoice(m, signal = null) {
  try {
    if (signal?.aborted) throw new Error("AbortError");
    
    // Ensure messages is always an array of objects
    const messagePayload = Array.isArray(m) ? m : [m];
    ...
    if (err.name === 'AbortError' || err.message === 'AbortError') throw err;
    console.error(`[Ollama] Choice Model Error (${CHOICE_MODEL}):`, err.message);
    throw err;
  }
}

async function getChoiceStream(m, signal = null) {
  try {
    if (signal?.aborted) throw new Error("AbortError");
    ...
    console.error(`[Ollama] Choice Stream Error (${CHOICE_MODEL}):`, err.message);
    throw err;
  }
}
module.exports = {
