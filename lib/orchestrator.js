...
{"search": boolean, "query": "optimized search query", "reason": "why"}`;
    ...
    const decisionRes = await aiProvider.getDecision(decisionPrompt, "You are Stable's brain. Respond ONLY in JSON.");
...
[Instructions]
You are an AI. Use the above context to answer the user query accurately.
...
Respond ONLY with the index of the best candidate (e.g., 0, 1, 2...).`;
  ...
  try {
    const decisionRes = await aiProvider.getDecision(decisionPrompt, "You are a judge of AI responses. Respond ONLY with a single integer.");
