...
async function orchestrate(apiMessages) {
  ...
  const aiProvider = getProvider();
  let N;
  if (aiProvider.numParallel && aiProvider.numParallel > 0) {
    N = aiProvider.numParallel;
    console.log(`[Orchestrator] OPENAI_NUM_PARALLEL=${N} override in effect`);
  } else {
    N = await calculateN();
  }
  console.log(`[Orchestrator] Generating ${N} candidates using ${aiProvider.choiceModel}...`);
